...
class Solution{
    ...
    public List<List<String>> solveNQueens(int n) {
        ...
        int[] pos=new int[n];
        size=(1<<n)-1;
        backTrace(0,pos,0,0,0);

        return res;
    }

    private void backTrace(int level,int[] pos,int col,int pie,int na){
        if(col==size) {
            res.add(convertToString(pos));
            return;
        }

        int extra=size & (~(col|pie|na));
        while(extra!=0){
            int p=extra & -extra;
            pos[level]=(int)(Math.log(p)/Math.log(2));
            backTrace(level+1,pos,col|p,(pie|p)<<1,(na|p)>>1);
            extra ^= p;
        }
    }

    private List<String> convertToString(int[] pos){
