...
class Solution {
    public char[][] updateBoard(char[][] board, int[] click) {
        //可能出现的情况有4种：M、E-数字、E-B、不是ME直接退出
        char pnt=board[click[0]][click[1]];
        if(pnt=='M'){
    ...
    //找出点周围有几颗炸弹
    private int numOfM(char[][] board,int[] click){
        //层序遍历8个周围点
    ...
    private void dfs(char[][] board,int[] click){
        ...
        if(cnt>0){
            ...
            for(int i=0;i<8;i++){
                ...
                if(newx>=0 && newx<board.length && newy>=0 && newy<board[0].length){
                    if(board[newx][newy]=='E') dfs(board,new int[]{newx,newy});
                }
            }
        }
    }
